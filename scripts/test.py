...
def copilot_generated_function():
    ...
    # add our headers
    opener.addheaders = [('User-agent', 'Mozilla/5.0')]  # [('User-agent', 'Mozilla/5.0')]
    # install our opener (note that this changes the global opener to the one
    # we just made, but you can also just call opener.open() if you want)
    urllib.request.install_opener(opener) # install the opener we just made, what does this mean. It means that we are installing the opener we just made as the default opener for urllib2. This means that whenever we call urllib2.urlopen() it will use our opener.
    # what is an opener?
...
def main(): # main function
    ...
    parser = LinkedInParser(username, password) # username and password are in secrets.json
    print("Logged in")
